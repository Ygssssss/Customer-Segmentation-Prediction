...
def run_ml_app():
    ...
    age = left.number_input('Customer Age', step=1, min_value=0, format='%d')
    ...
    work_experience = right.number_input('Customer Work Experience', step=1, min_value=0, format='%d')
    spending = left.selectbox('Spending', list(spending_mapping.keys()))
    family_size = right.number_input('Family Size', step=1, min_value=0, format='%d')
